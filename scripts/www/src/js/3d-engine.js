...
class Inspection3DEngine {
    constructor(canvasId) {
        ...
        this.defaultHotspotPositions = {
            // 前部
            hood: { x: 0, y: 1.3, z: 2.2, name: '引擎盖漆面' },
            frontBumper: { x: 0, y: 0.6, z: 2.8, name: '前保险杠漆面' },
            leftFender: { x: -1.2, y: 1.1, z: 2.0, name: '左前翼子板漆面' },
            rightFender: { x: 1.2, y: 1.1, z: 2.0, name: '右前翼子板漆面' },
            
            // 侧面 - 前门
            leftFrontDoor: { x: -1.3, y: 1.0, z: 0.8, name: '左前门漆面' },
            rightFrontDoor: { x: 1.3, y: 1.0, z: 0.8, name: '右前门漆面' },
            
            // 侧面 - 后门
            leftRearDoor: { x: -1.3, y: 1.0, z: -0.6, name: '左后门漆面' },
            rightRearDoor: { x: 1.3, y: 1.0, z: -0.6, name: '右后门漆面' },
            
            // 侧面 - 后翼子板
            leftRearFender: { x: -1.2, y: 1.1, z: -1.6, name: '左后翼子板漆面' },
            rightRearFender: { x: 1.2, y: 1.1, z: -1.6, name: '右后翼子板漆面' },
            
            // 后部
            trunk: { x: 0, y: 1.3, z: -2.3, name: '后备箱盖漆面' },
            rearBumper: { x: 0, y: 0.6, z: -2.8, name: '后保险杠漆面' },
            
            // 顶部
            roof: { x: 0, y: 1.6, z: -0.2, name: '车顶漆面' },
    ...
    updateHotspotStatus(pointKey, status) {
    ...
    loadGLTFModel(url, onLoad, onError) {
    ...
    loadExternalModel(url, onLoad, onError) {
        const extension = url.split('.').pop().toLowerCase();
        
        if (extension === 'fbx') {
            this.loadFBXModel(url, onLoad, onError);
        } else if (extension === 'gltf' || extension === 'glb') {
            this.loadGLTFModel(url, onLoad, onError);
        } else {
